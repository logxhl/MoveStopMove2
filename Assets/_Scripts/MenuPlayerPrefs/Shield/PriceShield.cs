...
public class PriceShield : MonoBehaviour
{
    ...
    private void Start()
    {
        for(int i = 0; i < shieldBtns.Count; i++)
        {
            int price = prices[i];
            int ind = i;
            shieldBtns[i].onClick.AddListener(() => OnItemClick(ind, price));
        }
    }
    ...
    {
        int selectInd = PlayerPrefs.GetInt("Shield_" + layerBtn, 0);
        if(selectInd == 1)
        {
    ...
    public void OnBuyBtnClick()
    {
        ...
        if(playerCoin >= currentPrice)
        {
            ...
            PlayerPrefs.SetInt("Shield_" + layerBtn, 1);
            Debug.Log("Da mua! Xu con lai: " + playerCoin);
            btnBuy.SetActive(false);
            btnAds.SetActive(false);
            btnSelect.SetActive(true);
        }
    ...
    public void ButtonSelect()
    {
        ...
        {
            PlayerPrefs.SetInt("SelectShieldBtn", layerBtn);
    ...
    private void Update()
    {
        int indBtn = PlayerPrefs.GetInt("SelectShieldBtn", -1);
        if(layerBtn == indBtn)
        {
            txtSelect.text = "Equipped";
            btnSelect.GetComponent<Image>().color = new Color(255f / 255f, 255f / 255f, 255f / 255f);
